...
namespace CronParser
{
    internal class ValidationUtility
    {
        ...
        public static int[] ValidateCollection(string cronValue, int max)
        {
            int[] result = cronValue.Split(',').Select(e => int.Parse(e)).ToArray();
            if (result.Any(i => i > max))
            {
                return null;
            }
        ...
        public static int[] ValidateStep(string cronValue, int max)
        {
        ...
        public static int[] ValidateRange(string cronValue, int max)
        {
            int[] values = cronValue.Split('-').Select(e => int.Parse(e)).ToArray();
            if(values.Length != 2)
            {
